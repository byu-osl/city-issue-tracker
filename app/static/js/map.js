var mapView = (function map_namespace(){
	...
	function createMarker(issue) {
		...
		var infowindow = new google.maps.InfoWindow({
			content: "<div><h1>" + issue.title + "</h1><p>" + issue.description + 
					"</p><a onclick='issuePopup.viewIssue(" + issue.id + ")'>Go to issue</a></div>"
		});
	...
	function setVisible(issue, visible){
		var id = issue.id != undefined ? issue.id : issue
